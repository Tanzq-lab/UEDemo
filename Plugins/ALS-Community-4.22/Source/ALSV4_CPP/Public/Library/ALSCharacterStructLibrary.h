...
USTRUCT(BlueprintType)
struct FALSCornerClimbValues
{
	GENERATED_BODY()

	/* 物体以及相对于物体的变换 */
	UPROPERTY(VisibleAnywhere, Category = "Ladge System")
	FALSComponentAndTransform CAT_TargetLS;
	
	/* 向内移动旋转位置移动曲线 */
	UPROPERTY(EditAnywhere, Category = "Ladge System")
	UCurveVector* MoveInnerCurve = nullptr;
	
	/* 向外移动旋转位置移动曲线 */
	UPROPERTY(EditAnywhere, Category = "Ladge System")
	UCurveVector* MoveOuterCurve = nullptr;
	 ...
	 * @brief 向内还是向外
	 */
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Ladge System")
	FALSCornerClimbType CornerType;
	...
	/**
	 * @brief 旋转方向值 向右为 1, 向左为 -1
	 */
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Ladge System")
	float DirectionValue;

	UCurveVector* GetMoveCurve() const
	{
		return CornerType.Inner() ? MoveInnerCurve : MoveOuterCurve;
	}
 ...
 */
USTRUCT(BlueprintType)
struct FALSClimbJumpParams
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, Category = "Climb System |Jump")
	float JumpGrowRate = 200.f;

	UPROPERTY(EditAnywhere, Category = "Climb System |Jump")
	float JumpLengthMax = 240.f;
};
