...
private:
	UPROPERTY(VisibleDefaultsOnly, BlueprintReadOnly, meta = (AllowPrivateAccess = true),
		Category = "ALS|Character States")
	EALSClimbingType Type = EALSClimbingType::Hanging;
...
public:
	...
	FALSClimbingType(const EALSClimbingType InitialClimbingType) { *this = InitialClimbingType; }

	const bool& Hanging() const { return Hanging_; }
	const bool& FreeHanging() const { return FreeHanging_; }

	operator EALSClimbingType() const { return Type; }

	void operator =(const EALSClimbingType NewType)
	{
		Type = NewType;
		Hanging_ = NewType == EALSClimbingType::Hanging;
		FreeHanging_ = NewType == EALSClimbingType::FreeHanging;
	}
};


USTRUCT(BlueprintType)
struct FALSCornerClimbType
{
	GENERATED_BODY()

private:
	UPROPERTY(VisibleDefaultsOnly, BlueprintReadOnly, meta = (AllowPrivateAccess = true),
		Category = "ALS|Climb States")
	EALSCornerClimbType Type = EALSCornerClimbType::Outer;
...
public:
	...
	FALSCornerClimbType(const EALSCornerClimbType InitialCornerClimbType) { *this = InitialCornerClimbType; }

	const bool& Outer() const { return Outer_; }
	const bool& Inner() const { return Inner_; }

	operator EALSCornerClimbType() const { return Type; }

	void operator =(const EALSCornerClimbType NewType)
	{
		Type = NewType;
		Outer_ = NewType == EALSCornerClimbType::Outer;
		Inner_ = NewType == EALSCornerClimbType::Inner;
	}
};
...
private:
	...
	UPROPERTY(VisibleDefaultsOnly, BlueprintReadOnly, meta = (AllowPrivateAccess = true),
		Category = "ALS|Movement System")
	bool CornerClimbing_ = false;

	UPROPERTY(VisibleDefaultsOnly, BlueprintReadOnly, meta = (AllowPrivateAccess = true),
		Category = "ALS|Movement System")
	bool ClimbJumping_ = false;
...
public:
	...
	const bool& CornerClimbing() const { return CornerClimbing_; }
	const bool& ClimbJumping() const { return ClimbJumping_; }
	...
	void operator=(const EALSMovementAction NewAction)
	{
		...
		CornerClimbing_ = Action == EALSMovementAction::CornerClimbing;
		ClimbJumping_ = Action == EALSMovementAction::ClimbJumping;
