...
void URootMotionModifier_AdjustmentBlendWarp::AdjustmentBlendWarp(const FBoneContainer& BoneContainer, const FCSPose<FCompactPose>& AdditivePose, const FMotionDeltaTrackContainer& MotionDeltaTracks, FAnimSequenceTrackContainer& Output)
{
	...
	for (const FCompactPoseBoneIndex PoseBoneIndex : AdditivePose.GetPose().ForEachBoneIndex())
	{
		...
		Track.PosKeys.Add(FVector(MotionDeltaTrack.BoneTransformTrack[0].GetLocation()));
		Track.RotKeys.Add(FQuat(MotionDeltaTrack.BoneTransformTrack[0].GetRotation()));
		...
		for (int32 FrameIdx = 1; FrameIdx < TotalFrames; FrameIdx++)
		{
			...
			Output.AnimationTracks[BoneIndex].PosKeys.Add(FVector(BoneTransform.GetTranslation() + CurrentAdditiveTranslation));
			...
			Output.AnimationTracks[BoneIndex].RotKeys.Add(FQuat(FRotator::MakeFromEuler(BoneTransform.GetRotation().Rotator().Euler() + CurrentAdditiveRotation).Quaternion()));
