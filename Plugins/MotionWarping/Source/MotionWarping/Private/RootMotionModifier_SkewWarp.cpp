...
void URootMotionModifier_SkewWarp::Update()
{
	Super::Update();
	...
	{
		CharacterOwner->SetActorLocation(FMath::VInterpTo(CharacterOwner->GetActorLocation(), GetTargetLocation(),
...
FTransform URootMotionModifier_SkewWarp::ProcessRootMotion(const FTransform& InRootMotion, float DeltaSeconds)
{
	...
	if (bWarpTranslation && !RootMotionDelta.GetTranslation().IsNearlyZero())
	{
		...
		const FVector CurrentLocation = (CharacterOwner->GetActorLocation() - CurrentRotation.GetUpVector() *
			CapsuleHalfHeight);
		...
		const FVector Translation = CharacterOwner->GetMesh()->ConvertLocalRootMotionToWorld(RootMotionDelta).
		                                            GetLocation();
		const FVector FutureLocation = CurrentLocation + CharacterOwner->GetMesh()->
	...
	if (DebugLevel == 2 || DebugLevel == 3)
	{
		...
		DrawDebugCoordinateSystem(CharacterOwner->GetWorld(), GetTargetLocation(), GetTargetRotator(), 50.f, false,
...
URootMotionModifier_SkewWarp* URootMotionModifier_SkewWarp::AddRootMotionModifierSkewWarp(
	UMotionWarpingComponent* InMotionWarpingComp, const UAnimSequenceBase* InAnimation, float InStartTime,
	float InEndTime,
	FName InWarpTargetName, EWarpPointAnimProvider InWarpPointAnimProvider, FTransform InWarpPointAnimTransform,
	FName InWarpPointAnimBoneName,
	bool bInWarpTranslation, bool bInIgnoreZAxis, bool bInWarpRotation, EMotionWarpRotationType InRotationType,
